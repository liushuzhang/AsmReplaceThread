...
import com.thread.plugin.check.CheckThreadClassModify
 ...
 *
 */
class ReplaceThreadTransform extends Transform {
    private static Project project
    private RepalceThreadExtension replaceThreadExtension

    ReplaceThreadTransform(Project project, RepalceThreadExtension replaceThreadExtension) {
        this.project = project
        this.replaceThreadExtension = replaceThreadExtension
    }

    @Override
    String getName() {
        return "replaceThreadTransform"
    }
    ...
    void _transform(Context context, Collection<TransformInput> inputs, TransformOutputProvider outputProvider, boolean isIncremnetal) throws IOException, TransformException, InterruptedException {
        ...
        inputs.forEach { TransformInput input ->
                ...
                if (dir) {
                    ...
                    dir.traverse(type: FileType.FILES, nameFilter: ~/.*\.class/) { File classFile ->
                        if (FilterClass.canModify(classFile.name)) {
                            //模块里的开关，默认是开启的
                            if (!replaceThreadExtension.disableReplaceThread) {
                                File modifiedFile = ReplaceThreadClassModify.modifyClassFile(dir, classFile, context.getTemporaryDir())
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                def modifyJar = null
                if (!replaceThreadExtension.disableReplaceThread) {
                    modifyJar = ReplaceThreadClassModify.modifyJar(jarInput.file, context.getTemporaryDir(), true)
                }
                if (modifyJar == null) {
