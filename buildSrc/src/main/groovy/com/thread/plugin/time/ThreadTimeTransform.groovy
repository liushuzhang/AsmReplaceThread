 ...
 *
 */
class ThreadTimeTransform extends Transform {
    private static Project project
    private ThreadTimeExtension threadTimeExtension

    ThreadTimeTransform(Project project, ThreadTimeExtension threadTimeExtension) {
        this.project = project
        this.threadTimeExtension = threadTimeExtension
    }

    @Override
    String getName() {
        return "threadTimeExtension"
    }
    ...
    void _transform(Context context, Collection<TransformInput> inputs, TransformOutputProvider outputProvider, boolean isIncremnetal) throws IOException, TransformException, InterruptedException {
        ...
        inputs.forEach { TransformInput input ->
                ...
                if (dir) {
                    ...
                    dir.traverse(type: FileType.FILES, nameFilter: ~/.*\.class/) { File classFile ->
                        if (FilterClass.canModify(classFile.name)) {
                            //模块里的开关，默认是开启的
                            if (!threadTimeExtension.disableCheckThread) {
                                File modifiedFile = ThreadTimeClassModify.modifyClassFile(dir, classFile, context.getTemporaryDir())
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                def modifyJar = null
                if (!threadTimeExtension.disableCheckThread) {
                    modifyJar = ThreadTimeClassModify.modifyJar(jarInput.file, context.getTemporaryDir(), true)
                }
                if (modifyJar == null) {
