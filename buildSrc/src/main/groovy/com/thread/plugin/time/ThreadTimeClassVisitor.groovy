...
class ThreadTimeClassVisitor extends ClassVisitor implements Opcodes {
    //类名
    ...
    private boolean lambdaRun;

    ThreadTimeClassVisitor(final ClassVisitor classVisitor, String jarName) {
    ...
    void visit(int version, int access, String name, String signature, String superName, String[] interfaces) {
        ...
        if ((access & Opcodes.ACC_SUPER) > 0) {
            if (superName == ClassConstant.M_Thread) {
                isRunable = true
            }
            if (interfaces != null && interfaces.size() > 0) {
                if (interfaces.contains(ClassConstant.M_Runable)) {
                    isRunable = true
                }
            }
        }
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        System.out.println("---visitMethod-----" + nameDesc)

//        if (isRunable && name == "run") {
        mv = new ThreadTimeMethodVisitor(ASM6, mv, jarName, className, name, (isRunable && name == "run") || lambdaRun, nameDesc)
//        }
        return mv
    }
